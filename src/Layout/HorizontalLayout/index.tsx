...
import { menuUser } from "../LayoutMenuUser";
...
const HorizontalLayout = (props: any) => {
  ...
  const [menuItems, setMenuItems] = React.useState<any>(menuUser);
  ...
  React.useEffect(() => {
    const handleResize = () => {
      ...
      const currentMenu = user.user.role === "admin" ? menuData : menuUser;

      for (let i = 0; i < currentMenu.length; i++) {
        const itemWidth = navbarNav?.children[i]?.offsetWidth;
        totalItemsWidth += itemWidth;
        if (totalItemsWidth <= menuWidth - 50 || window.innerWidth < 768) {
          visibleItems.push(currentMenu[i]);
        } else {
          hiddenItems.push(currentMenu[i]);
        }
