...
const NewAssetMasukPage = () => {
  const [data, setData] = useState<any>([]);
  const [Asset, setAsset] = useState<any>([]);
  const [eventData, setEventData] = useState<any>();
  console.log("🚀 ~ NewAssetMasukPage ~ eventData:", eventData);
  ...
  const handleDelete = () => {
    if (eventData) {
      handleDeleteDataAssetMasuk(eventData.id);
  ...
  const validation: any = useFormik({
    ...
    initialValues: {
      ...
      asset_id: (eventData && eventData.asset_id) || "",
    ...
    onSubmit: (values) => {
      ...
      if (isEdit) {
        handleUpdateAssetMasuk(values);
      } else {
        handlePostAssetMasuk(values);
      }
      if (isLoading) {
        toggle();
      }
  ...
  const fetchDataAssetMasuk = async () => {
    setLoadingV(true);
    try {
      ...
      setAsset(userResponse.data.data.data);
    } catch (error: any) {
  ...
  const fetchNewDataAssetMasuk = async () => {
    setLoadingV(true);
    try {
      const userResponse = await axiosInstance.get("/api/asset-masuk", {
        headers: {
          Authorization: `Bearer ${user.token}`,
        },
      });
  ...
  const handlePostAssetMasuk = async (data: any) => {
    try {
      setIsLoading(true);
  ...
  const handleUpdateAssetMasuk = async (data: any) => {
    try {
      setIsLoading(true);
      const formData = new FormData();
      formData.append("asset_id", eventData.asset_id.id || data.asset_id);
      formData.append("category", data.category);
      formData.append("kondisi", data.kondisi);
      formData.append("quantity", data.quantity);
      formData.append("tanggal_masuk", data.tanggal_masuk);
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div className="shrink-0">
              ...
              >
                ...
                <span className="align-middle">Add Asset Masuk</span>
              </Link>
            </div>
          </div>
          {data && data.length > 0 ? (
            ...
            <div className="grid grid-cols-1 gap-4 xl:grid-cols-12">
              <div className="xl:col-span-12">
                ...
                <select
                  ...
                  value={
                    validation.values.asset_id.id ||
                    (isEdit && eventData && eventData.asset_id.id) ||
                    ""
                  }
                >
...
export default NewAssetMasukPage;
