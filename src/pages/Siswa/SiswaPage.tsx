...
const SiswaPage = () => {
  ...
  const handleDelete = () => {
    if (eventData) {
      handleDeleteDataSiswa(eventData.id);
  ...
  const validation: any = useFormik({
    ...
    initialValues: {
      ...
      npm: (eventData && eventData.npm) || "",
      kelas: (eventData && eventData.kelas) || "",
      umur: (eventData && eventData.umur) || "",
    },
  ...
  const fetchDataUser = async () => {
    setLoadingV(true);
    try {
      const userResponse = await axiosInstance.get("/api/siswa", {
        headers: {
  ...
  const handlePostSiswa = async (data: any) => {
    try {
      setIsLoading(true);
      // upload file
      ...
      const userResponse = await axiosInstance.post("/api/siswa", formData, {
        headers: {
          Authorization: `Bearer ${user.token}`,
  ...
  const handleDeleteDataSiswa = async (id: any) => {
    try {
      setIsLoading(true);
      const userResponse = await axiosInstance.delete(`/api/siswa/${id}`, {
        headers: {
          Authorization: `Bearer ${user.token}`,
        },
      });
  ...
  return (
    <Layout>
      <BreadCrumb title="Data Siswa" pageTitle="Users" />
      <DeleteModal
        show={deleteModal}
        onHide={deleteToggle}
        ...
        <div className="card-body">
          <div className="flex items-center gap-3 mb-4">
            <h6 className="text-15 grow">
              Siswa (<b className="total-Employs">{data.length}</b>)
            </h6>
            <div className="shrink-0">
              ...
              >
                ...
                <span className="align-middle">Add Siswa</span>
              </Link>
            </div>
          </div>
          {data && data.length > 0 ? (
        ...
        <Modal.Body className="max-h-[calc(theme('height.screen')_-_180px)] p-4 overflow-y-auto">
          ...
          >
            ...
            <div className="grid grid-cols-1 gap-4 xl:grid-cols-12">
              <div className="xl:col-span-12">
                ...
                <input
                  ...
                  placeholder="NPM Siswa"
                  name="npm"
                  onChange={validation.handleChange}
                  value={validation.values.npm || ""}
                />
                {validation.touched.npm && validation.errors.npm ? (
                  <p className="text-red-400">{validation.errors.npm}</p>
                ) : null}
              </div>
            </div>
            <div className="grid grid-cols-1 gap-4 xl:grid-cols-12">
            ...
            <div className="grid grid-cols-1 gap-4 xl:grid-cols-12">
              <div className="xl:col-span-12">
                ...
                <input
                  ...
                  name="umur"
                  onChange={validation.handleChange}
                  value={validation.values.umur || ""}
                />
                {validation.touched.umur && validation.errors.umur ? (
                  <p className="text-red-400">{validation.errors.umur}</p>
                ) : null}
              </div>
            </div>
            <div className="flex justify-end gap-2 mt-4">
              ...
              >
                {isLoading ? "Loading" : !!isEdit ? "Update" : "Add Siswa"}
              </button>
            </div>
          </form>
        </Modal.Body>
