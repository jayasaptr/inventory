...
const RuanganPage = () => {
  ...
  const handleDelete = () => {
    if (eventData) {
      handleDeleteDataCategory(eventData.id);
  ...
  const validation: any = useFormik({
    ...
    initialValues: {
      ...
      id_user: (eventData && eventData.id_user) || "",
    },
    validationSchema: Yup.object({
      nama: Yup.string().required("Nama is required"),
      id_user: Yup.string().required("Kepala Ruangan is required"),
    }),

    onSubmit: (values) => {
      if (isEdit) {
        handleUpdateDataCateogry(values);
      } else {
        handlePostCategory(values);
      }
      if (isLoading) {
        toggle();
      }
  ...
  const fetchDataRuangan = async () => {
    ...
    try {
      ...
      Error("Data Ruangan gagal diambil");
  ...
  const handlePostCategory = async (data: any) => {
    try {
      setIsLoading(true);
      ...
      const userResponse = await axiosInstance.post("/api/ruangan", formData, {
        headers: {
          Authorization: `Bearer ${user.token}`,
      ...
      if (userResponse.data.success === true) {
        Success("Data Ruangan berhasil ditambahkan");
        fetchDataRuangan();
        toggle();
      }
    } catch (error: any) {
      Error("Data Ruangan gagal ditambahkan");
      if (error.response.status === 401) {
        localStorage.removeItem("authUser");
        naviagate("/login");
      }
  ...
  const handleUpdateDataCateogry = async (data: any) => {
    try {
      setIsLoading(true);
      // upload file
      const formData = new FormData();
      ...
      if (userResponse.data.success === true) {
        Success("Data Ruangan berhasil diupdate");
        fetchDataRuangan();
        toggle();
      }
    } catch (error: any) {
      Error("Data Ruangan gagal diupdate");
      if (error.response.status === 401) {
        localStorage.removeItem("authUser");
        naviagate("/login");
      }
  ...
  const handleDeleteDataCategory = async (id: any) => {
    try {
      setIsLoading(true);
      const userResponse = await axiosInstance.delete(`/api/ruangan/${id}`, {
        headers: {
          Authorization: `Bearer ${user.token}`,
        },
      });
      ...
      Error("Data Ruangan gagal dihapus");
      if (error.response.status === 401) {
        localStorage.removeItem("authUser");
        naviagate("/login");
      }
  ...
  const fetchDataUser = async (page = 1) => {
    // fetch for data react-infinite-scroll
    try {
      const userResponse = await axiosInstance.get(`/api/user?page=${page}`, {
        headers: {
          Authorization: `Bearer ${user.token}`,
        },
      });
      console.log("🚀 ~ fetchDataUser ~ userResponse:", userResponse.data);

      if (userResponse.data.data.data.length > 0) {
        const newOptions = userResponse.data.data.data.map((item: any) => {
          return {
            value: item.id,
            label: item.name,
          };
  ...
  const handleSelectChange = (selectedOption: any) => {
    validation.setFieldValue("id_user", selectedOption.value);
  };
  ...
  return (
    <Layout>
      <BreadCrumb title="Data Ruangan" pageTitle="Users" />
      <DeleteModal
        show={deleteModal}
        onHide={deleteToggle}
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div className="shrink-0">
              ...
              >
                ...
                <span className="align-middle">Add Ruangan</span>
              </Link>
            </div>
          </div>
          {data && data.length > 0 ? (
      ...
      >
        ...
        >
          <Modal.Title className="text-16">
            {!!isEdit ? "Edit Kondisi" : "Add Kondisi"}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body className="max-h-[calc(theme('height.screen')_-_180px)] p-4 overflow-y-auto">
          ...
          >
            ...
            <div className="grid grid-cols-1 gap-4 xl:grid-cols-12">
              ...
              <div className="xl:col-span-12">
                ...
                <select
                  ...
                  name="id_user"
                  onChange={validation.handleChange}
                  value={validation.values.id_user || ""} // set default value
                >
            ...
            <div className="flex justify-end gap-2 mt-4">
              ...
              >
                {isLoading ? "Loading" : !!isEdit ? "Update" : "Add Kondisi"}
              </button>
            </div>
          </form>
        </Modal.Body>
