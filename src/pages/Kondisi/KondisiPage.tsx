...
const KondisiPage = () => {
  ...
  const handlePostCategory = async (data: any) => {
    try {
      ...
      const userResponse = await axiosInstance.post("/api/kondisi", formData, {
        headers: {
          Authorization: `Bearer ${user.token}`,
  ...
  const handleDeleteDataCategory = async (id: any) => {
    try {
      setIsLoading(true);
      const userResponse = await axiosInstance.delete(`/api/kondisi/${id}`, {
        headers: {
          Authorization: `Bearer ${user.token}`,
        },
      });
  ...
  return (
    <Layout>
      <BreadCrumb title="Data Kondisi" pageTitle="Users" />
      <DeleteModal
        show={deleteModal}
        onHide={deleteToggle}
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div className="shrink-0">
              ...
              >
                ...
                <span className="align-middle">Add Kondisi</span>
              </Link>
            </div>
          </div>
          {data && data.length > 0 ? (
