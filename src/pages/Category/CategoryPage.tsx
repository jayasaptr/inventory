...
const CategoryPage = () => {
  ...
  const handlePostCategory = async (data: any) => {
    try {
      ...
      const userResponse = await axiosInstance.post("/api/category", formData, {
        headers: {
          Authorization: `Bearer ${user.token}`,
  ...
  const handleDeleteDataCategory = async (id: any) => {
    try {
      setIsLoading(true);
      const userResponse = await axiosInstance.delete(`/api/category/${id}`, {
        headers: {
          Authorization: `Bearer ${user.token}`,
        },
      });
  ...
  return (
    <Layout>
      <BreadCrumb title="Data Category" pageTitle="Users" />
      <DeleteModal
        show={deleteModal}
        onHide={deleteToggle}
        ...
        <div className="card-body">
          <div className="flex items-center gap-3 mb-4">
            <h6 className="text-15 grow">
              Category (<b className="total-Employs">{data.length}</b>)
            </h6>
        ...
        <Modal.Body className="max-h-[calc(theme('height.screen')_-_180px)] p-4 overflow-y-auto">
          ...
          >
            ...
            <div className="flex justify-end gap-2 mt-4">
              ...
              >
                {isLoading ? "Loading" : !!isEdit ? "Update" : "Add Category"}
              </button>
            </div>
          </form>
        </Modal.Body>
